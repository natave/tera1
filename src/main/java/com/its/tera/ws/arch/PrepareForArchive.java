...
public class PrepareForArchive extends RootWebScript{
	
	private static Log logger = LogFactory.getLog(PrepareForArchive.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		...
		aspectProperties.put(CM.QNAME_PROP_ASPECT_ARCHIVE_USER, AuthenticationUtil.getFullyAuthenticatedUser());
		
		logger.debug("preparedUser = " + AuthenticationUtil.getFullyAuthenticatedUser() + ", refs = " + nodeRefs);
		...
		for (String string : nodeRefs) {
					...
					if(aagr != null) {
						...
						if(nodeRefs.contains(gAgrRef.toString())) {
							if(!nodeService.hasAspect(nodeRef, CM.QNAME_ASPECT_FOR_ARCHIVE)) {
								nodeService.addAspect(nodeRef, CM.QNAME_ASPECT_FOR_ARCHIVE, aspectProperties );
			...
			else if(nodeService.getType(nodeRef).equals(CM.QNAME_TYPE_GENERAL_AGREEMENT_FOLDER) && !nodeService.hasAspect(nodeRef, CM.QNAME_ASPECT_FOR_ARCHIVE))
			{
				...
				logger.debug(" is not agreement or already with archive aspect = " + nodeRef);
			}			
		}
