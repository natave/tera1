...
public class ArchiveAgreements extends RootWebScript{
	
	private static Log logger = LogFactory.getLog(ArchiveAgreements.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		...
		for (NodeRef nodeRef : nodeRefs) {
			...
			if(!nodeService.hasAspect(clientRef, CM.QNAME_ASPECT_HAS_ARCHIVE)) {
				logger.debug("ADD_HAS_ARCHIVE clientRef" + clientRef.toString() + " - " + (String)nodeService.getProperty(clientRef, CM.QNAME_PROP_ASPECT_CLIENT_CODE));
				nodeService.addAspect(clientRef, CM.QNAME_ASPECT_HAS_ARCHIVE, null);
			}
			else
			{
				logger.debug("Not_ADD_HAS_ARCHIVE clientRef" + clientRef.toString() + " - " + (String)nodeService.getProperty(clientRef, CM.QNAME_PROP_ASPECT_CLIENT_CODE));
			}
			...
			for (ChildAssociationRef childDoc : childDocs) {
				TechModel docModel = FillModelsUtil.getDocModelFull(childDoc.getChildRef(), clientArchiveRef, serviceRegistry, "/alfresco/service", appParameters);
			...
			for (AssociationRef genChildAssoc : genChildAssocs) {
				...
				for (ChildAssociationRef agrChildDoc : agrChildDocs) {
					...
					TechModel docModel = FillModelsUtil.getDocModelFull(agrChildDoc.getChildRef(), clientArchiveRef, serviceRegistry, "/alfresco/service", appParameters);
		 ...
		 String query1 = "TYPE:\"" + CM.QNAME_TYPE_AGREEMENT_FOLDER + "\" AND ASPECT:\""+ CM.QNAME_ASPECT_FOR_ARCHIVE + "\" AND -EXISTS:\""/* "ISNULL:\"" */ + CM.QNAME_PROP_ASPECT_GEN_AGR_NUM + "\"";	
		...
		logger.debug("result1 length = " + resultSet1.length());
		 ...
		 for (NodeRef nodeRef : nodeRefs1) {
			 ...
			 //CLIENT REGISTRATION
			 List<AssociationRef> sourceAssocs = nodeService.getSourceAssocs(nodeRef, CM.QNAME_ASSOC_ASPECT_AGRS); 
			 if(sourceAssocs == null || sourceAssocs.isEmpty()) {
				 logger.debug("sourceAssocs == null,  agr number = " + (String)nodeService.getProperty(nodeRef, CM.QNAME_PROP_ASPECT_AGR_NUM) + ", noderef = " + nodeRef.toString());
				 continue;
			 }
			 NodeRef clientRef = sourceAssocs.get(0).getSourceRef();
