...
public class GAgrReg extends RootWebScript{
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		...
		Long regDate = agr.getRegDate() != null ? agr.getRegDate(): agr.getCreated();
		NodeRef monthRef;
		try {
			monthRef = NodeUtil.getMonthRef(new Date(regDate), agr.getBranch(), false, 
...
//		Map<String, Serializable>  props = agr.getPropertiesMap();
		...
		List<String> histList = agr.getArchHistoryString() == null ? new ArrayList<String>() : agr.getArchHistoryString();
		String parameterValue = "live" + "#" + agr.getLiveUser() + "#" + date + "#" + "";
		histList.add(parameterValue);
		agr.setArchHistoryString(histList);
		agr.setHistoryDate(date);
		
		logger.debug("histList value = " + parameterValue);
...
//		}
		//propsQnames.put(ContentModel.PROP_NAME, CO.GAGR_PREFIX + new Date().getTime());
		...
		ChildAssociationRef agreement = nodeService.createNode(monthRef, ContentModel.ASSOC_CONTAINS, 
				QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, CO.GAGR_PREFIX + agr.getName()),
				CM.QNAME_TYPE_GENERAL_AGREEMENT_FOLDER, propsQnames);
		NodeRef agreementRef = agreement.getChildRef();logger.debug("agreementRef = " + agreementRef);
