...
public class AgreementsArchiveSend {
	
	private static Log logger = LogFactory.getLog(AgreementsArchiveSend.class);
	...
	protected void execute() {
		...
		for (NodeRef nodeRef : nodeRefs) {
			...
			for (ChildAssociationRef childDoc : childDocs) {
				TechModel docModel = FillModelsUtil.getDocModelFull(childDoc.getChildRef(), clientArchiveRef, services, "/alfresco/service", appParameters);
				...
				if(nodeService.getProperty(childDoc.getChildRef(), ContentModel.PROP_CONTENT) != null) {
					InputStream stream = services.getContentService().getReader(childDoc.getChildRef(), ContentModel.PROP_CONTENT).getContentInputStream();
			...
			for (AssociationRef genChildAssoc : genChildAssocs) {
				...
				for (ChildAssociationRef agrChildDoc : agrChildDocs) {
					...
					TechModel docModel = FillModelsUtil.getDocModelFull(agrChildDoc.getChildRef(), clientArchiveRef, services, "/alfresco/service", appParameters);
					...
					if(nodeService.getProperty(agrChildDoc.getChildRef(), ContentModel.PROP_CONTENT) != null) {
						InputStream stream = services.getContentService().getReader(agrChildDoc.getChildRef(), ContentModel.PROP_CONTENT).getContentInputStream();
						byte[] binaryData = null;
						try {
							binaryData = IOUtils.toByteArray(stream);
						} catch (IOException e1) {
